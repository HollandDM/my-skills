...
returns 1 while any ledger error remains.
  ...
  terminal  <dir> D-NNN "<target>: <identifier>"      leaf: what implements it (own unwritten code included)
...
def check(led: Ledger) -> None:
    ...
    for nid, n in nodes.items():
        ...
        for it in stmts:
            ...
            if (
                n["design"] not in ("superseded", "retired")
                and (cid := it.get("child")) in nodes
                and nodes[cid]["design"] == "superseded"
            ):
...
def v_terminal(led: Ledger, a) -> int:
    ...
    n["target"] = t
    return finish(led, f"{a.id} terminal ⇒ {t}. JSON-set one adaptation line per contract clause — a clause with no nameable "
                       f"construct means this is not a terminal — then run `proposal {a.id}` before approval.")

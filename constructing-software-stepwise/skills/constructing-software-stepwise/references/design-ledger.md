...
```text
docs/design/<topic>/
  DESIGN.md                GENERATED by `stepwise.py sync`: index (root, frontier, ADRs, node table) + Program
...
- **Self-describing header.** `Kind`, ID, `Index` link, three status axes, `Parents`, `Depends on`. Write bare names (`D-010`, `CTX-F03`, `Run Key`, `ADR-0003`); `sync` links them.
- **Name, never repeat.** Contract cites context entries by name. Never restate definitions.
...
- Node file written by agent; `DESIGN.md` row, frontier, `Program` written by `sync`. Never hand-edit `DESIGN.md`.

Node born via `stepwise.py new <dir> D-NNN` as `Design: draft` (Statement + Parents prefilled): agent adds Effect + Contract (≤ 6 clauses), unknowns `?slug`. Interview replaces each `?` with a name in `Depends on`. Approval flips `draft` → `approved`, sets `Approved: <date> by user`, fills Refinement / Composition / Decisions / Deferred; `sync` substitutes body into `Program`. Realization + Evidence filled at implementation. Re-approval keeps `approved`, bumps `Approved:` — no `approved (revised)`.

Only `approved` authoritative. `draft` = scope fence for interview. Candidate body lives in Proposal block until approved.

## Index — `DESIGN.md` (generated)

Shape below is what `sync` writes; read it, never edit it.
...
Rows, `Active frontier`, ADR list all derive from node files, `adr/`, and body tags. Stable IDs (`D-NNN`) from first node. ID = statement; never renumber.
...
- approved body → substituted under its statement line; draft / frontier / stale / superseded → tag from header status.
- terminal → `⇒ <target>: <identifier>`; `Verification: verified` → `✓`.
- one parent whose body calls it → inline. Two or more parents, or parent terminal (body handed to platform by reference, e.g. `DBOS.startWorkflow(execute_run, …)`) → once under `### Procedures` with `(used by …)`, call sites `↗ D-NNN`.
- multi-line calls joined to one statement.
- Drift impossible by construction; `check` fails when `DESIGN.md` was hand-edited.
...
Parents: D-000
Depends on: CTX-F03, Run Key, D-010, ADR-0003

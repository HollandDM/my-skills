...
Initialization is not completion: the CLI does not analyze source. Continue with source inspection and batches of `adopt`, `bind`, and `observe` in the new directory. Pseudocode is the primary deliverable: write source-backed algorithms that explain the selected implementation and explicitly link substantial operations to their procedures or implementation boundaries. Read the quality contract and required review passes in [pseudocode.md](pseudocode.md). Establish new intended contracts and approvals only when authorized. With no previous ledger, start directly with `adopt`.

Read applicable requirements and accepted architectural constraints while deriving the topology independently. Inventory decisions from both the current repository and the previous design; do not stop at the previous ledger's ADR list. Rebuilt ledgers keep their ADRs under `<new-dir>/adr/`, so old ADR node IDs cannot silently constrain unrelated new nodes. Map applicable decisions to the rebuilt responsibilities and record their source. Record terminology, relevant failure/concurrency scenarios, and unresolved conflicts in the ledger with owning-node dependencies. Do not discard a requirement because code omits it or call an application behavior fixed because its documentation was clarified.
...
`observed_children` alone does not connect an algorithm. Put `▷ D-NNN: explanation` on the actual call or dispatch line, or link the actual asynchronous interaction in the behavior diagram. Keep operation names consistent across procedure signatures and call sites; explain an intentional dispatch-to-handler mapping. A reader must not have to guess which child implements a line.
...
`scan --json` is read-only. In a source-backed model it reports every active node, including nodes never bound to source, plus current, recorded, and observed versions; changed bindings; inspection tokens; conformance; and notifications. Its `coverage` object counts active, bound, observed, and current nodes and lists unbound IDs. Its separate `pseudocode` object reports algorithm traceability gaps, with node, basis, displayed line, and possible procedure matches where known. Source freshness can be complete while pseudocode is incomplete. `pending: []` is meaningful only when `coverage.complete` is true. Run a scan when beginning or resuming this workflow and before relying on reconstructed nodes. There is no background watcher; the signal is refreshed on CLI reads/writes and HTML export.
...
An observation requires an effect and nonempty claims. Each claim cites this node's binding IDs and labels its basis `observed` or `inferred`. Use `unknowns` for unresolved requirements or runtime assumptions. The CLI permits partial observations without pseudocode while inspection is underway; a complete reconstruction must explain its procedures with pseudocode. Observed algorithms and optional behavior diagrams remain separate from intended bodies and diagrams. The inspection is attributed through `--by` (default `agent inspection`). Observation revisions preserve earlier claims, source bindings, and implementation fingerprints.

Before updating an observation, run `observation <dir> D-NNN` to export a round-trippable payload. With unchanged source and design, omitting existing pseudocode, behavior, unknowns, or comparisons preserves them. After source or design changes, fields that could be stale must be supplied explicitly; omission is rejected. Supply `"pseudocode":""`, `"unknowns":[]`, `"behavior":{}`, or `"comparisons":{}` to clear a field deliberately. Stored parsed `body` is output as editable `pseudocode`; never filter the stored observation object and resubmit it directly.
...
Adoption is complete when the selected behavior is explained by current source-backed algorithms, substantial operations have explicit drill-down references or implementation mappings, relevant relationships are modeled, and uncertainties are explicit. Require `coverage.complete`, `pseudocode.complete`, `check --strict-pseudocode`, and the top-down and source-correspondence review passes. The absence of intended contracts does not prevent completion. Reconstruction does not need a fully approved design frontier.

Sync is complete when affected pseudocode has been reinspected against implementation and its references updated, or remaining inaccessible/unresolved areas are explicitly reported, and differences from intended contracts remain visible. Do this in the same task as the implementation change. `scan` lists pending inspections, algorithm gaps, and recorded differences. Missing sources and stale algorithms cannot be called current merely to finish the workflow.

...
`scan --json` is read-only. In a source-backed model it reports every active node, including nodes never bound to source, plus current, recorded, and observed versions; changed bindings; inspection tokens; conformance; and notifications. Its `coverage` object counts active, bound, observed, and current nodes and lists unbound IDs. `pending: []` is meaningful only when `coverage.complete` is true. Run a scan when beginning or resuming this workflow and before relying on reconstructed nodes. There is no background watcher; the signal is refreshed on CLI reads/writes and HTML export.

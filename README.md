...
### [constructing-software-stepwise](./constructing-software-stepwise/) `v4.1.1`
...
Pseudocode is the primary artifact and stays closely aligned with implementation through required readability and source-correspondence reviews. `check --strict-pseudocode` detects missing algorithms, disconnected helper calls, and unexplained relationships independently of source freshness. Before every user-facing Stepwise question or approval request, agents persist review context and export a fresh linked HTML snapshot. The reader opens pseudocode first, makes entire referenced call lines navigable with source-preserving browser history, shows implementation mappings and unresolved questions alongside algorithms, and provides a resizable TOC with keyboard controls and saved width.

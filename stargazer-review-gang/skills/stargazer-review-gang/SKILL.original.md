...
Store as `SKILL_DIR` — all file references below (agents/, reviewers/) relative to it.

**Say exactly:** "Starting the stargazer-review-gang."

**Then immediately proceed to Step 1.** No diff gathering. No file reading. Nothing else before Step 1.
...
2. **YOU DO NOT READ DIFFS, SOURCE FILES, OR AGENT INSTRUCTION FILES.** No
   ...
   Orchestrator determines diff ref from user's review scope.
...
No asking user for context. Infer from current session:
...
Use `model: "sonnet"` — orchestrator interprets scope, determines diff refs, reads
all diffs, routes files. One-shot job, no persistence needed.

**Pass user's scope verbatim** — no interpreting into base/head refs. Orchestrator
determines correct git diff strategy itself. **Add surrounding context** (branch
...
Use **TeamCreate** to create team for this review session:
...
Related concerns combined into single file per group.
...
filters, and produces final report. No additional validators after — its output is final.

**Present validator's report to user verbatim.** No rewriting, reformatting, summarizing,
or stripping — including severity emoji indicators (🔴🟡🔵), code blocks, confidence
scores, reviewer attributions. Report = deliverable; pass it through.
...
If only nitpicks, skip this step entirely. Otherwise use **AskUserQuestion** tool:
...
issues. They already have full file context from their review, making fixes more accurate.

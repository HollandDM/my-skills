...
| 6 | Tapir Endpoints | server + client endpoints + **public API hard-block** | Server: `EndpointServer`, `AuthenticatedEndpoint`, `authRoute`, `validateRoute` in `/jvm/`. Client: `EndpointClient`, `AuthenticatedEndpointClient`, `AsyncEndpointClient` in `/js/`. Also: server wiring files (`*Server.scala` in `apps/`) registering `.services` or `.asyncServices`. **PUBLIC API** (always route here, BLOCKER scope): any path under `modules/brienne/**/*PublicApiEndpoints.scala`, `modules/brienne/**/*PublicApiServer.scala`, `modules/brienne/**/PublicApiServerModule.scala`, `modules/brienne/**/PublicApiTapirServerAuthentication.scala`, `apps/gondor/**/GondorPublicApiSchema*.scala`, `js/public-api-specs/**`; also any diff containing `extends PublicApiEndpoints`, `extends PublicApiTapirServerAuthentication`, `publicApiEndpoint`, `publicApiRoute`, `BasePublicApiEndpoint`, `publicAPIServices`, or Tapir path starting `"api" / "v1"` |
...
10. **Public API hard-block (group 6)**: ALWAYS route to group 6 if any changed file matches public API path heuristics (see group 6 trigger). This work team NEVER edits public API — past incident broke external consumers. Include group 6 even if file is `js/public-api-specs/**` (otherwise non-Tapir). Mark in `workload` so reviewer prioritises Part 0 of checklist.

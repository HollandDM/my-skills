...
Reviewers = active team members — message directly. They have full file context, can
answer questions, provide fixes, confirm/withdraw findings. Use throughout validation,
not just as last resort.
...
Batch multiple questions per reviewer into one message. Wait for response before
...
Rules:
- **FALSE_POSITIVE**: line doesn't exist, wasn't changed in diff, issue already handled
...
2. **RETAIN everything >= 50** — MUST keep every non-duplicate >= 50. Cannot drop,
   downgrade, or omit for being minor, borderline, or stylistic. >= 50 + not duplicate
   → goes in report.
...
No rewriting, summarizing, shortening, or paraphrasing. Reviewer has full file context
